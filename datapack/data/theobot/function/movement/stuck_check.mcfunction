# Position-based stuck detection at the current bot position.
...
execute if score @a[tag=theobot,distance=..0.2,limit=1] tb.dx matches -2..2 if score @a[tag=theobot,distance=..0.2,limit=1] tb.dz matches -2..2 if score @a[tag=theobot,distance=..0.2,limit=1] tb.state matches 1..5 run scoreboard players add @a[tag=theobot,distance=..0.2,limit=1] tb.stuck 1
